...
def cartoon_consumption_history(args):
    pass


def get_result(order_histories, cartoon_info):
    return get_output_data(get_order_data_list(cartoon_info, order_histories), order_histories)
...
def get_order_data_list(cartoon_info, order_histories):
    ...
    for order_history in order_histories:
        cartoon_consumption_histories = []
        for cartoon_consumption_histories in order_history["cartoon_consumption_histories"]:
...
def get_output_data(order_data_list, order_histories):
    result = ""

    for order_data in order_data_list:
        result += f"{order_data['customer']} 주문 내역\n"  # 출력 결과 추가하기
        for cartoon_consumption_history in order_data['cartoon_consumption_histories']:
            result += f"{cartoon_consumption_history['cartoon_name']} : {cartoon_consumption_history['amount']} : {cartoon_consumption_history['view_count']}"
...
def calculate_point_from_history(cartoon_info, order_history):
    point = 0  # 포인트 계산하기
    for cartoon_consumption_history in order_history["cartoon_consumption_histories"]:  # 각 만화 소비 기록별 for문
        point += calculate_point(cartoon_consumption_history, cartoon_info, point)
    return point


def calculate_point(cartoon_consumption_history, cartoon_info, point):
    point = 0
